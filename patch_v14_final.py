...
def convert_to_json_patch(instructions, base_doc):
    ...
    for op_group in instructions.get('operations', []):
        for action in op_group.get('actions', []):
            ...
            path_exists = True
            try:
                pointer.resolve(base_doc)
            except jsonpatch.JsonPointerException:
                path_exists = False

            if action_type in ['add_or_merge', 'update_value']:
                op['op'] = 'replace' if path_exists else 'add'
                op['path'] = path
                op['value'] = value
                patch.append(op)
                ...
                patch.append({'op': 'add', 'path': f"{path}/-", 'value': value})
...
def main():
    ...
    # --- Pass 1: Apply v13 enhancements directly ---
    ...
    try:
        ...
        final_data = jsonpatch.apply_patch(data, patch_ops)
        print("v14 enhancements applied successfully via jsonpatch.")
    except Exception as e:
