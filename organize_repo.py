...
def create_new_tree(base_tree_sha, all_files):
    """Dosya taşıma işlemlerini yansıtan bir değişiklik ağacı oluşturur."""
    new_tree = []
    moved_files = 0
    for file_info in all_files:
        path = file_info["path"]

        # Yalnızca taşınması gereken dosyalara (bloblara) odaklan
        if file_info["type"] != "blob":
            continue

        new_path = None
        if path in CORE_FILES:
            # Bu dosya project_core klasörüne taşınmalı
            new_path = f"project_core/{path}"
        elif path not in IGNORE_FILES:
            # Bu dosya archive klasörüne taşınmalı
            new_path = f"archive/{path}"

        # Eğer dosya için yeni bir yol belirlendiyse, taşıma işlemi gerekir
        if new_path and new_path != path:
            moved_files += 1
            # 1. Yeni dosya yolunu orijinal içerik SHA'sı ile ekle
            new_tree.append({
                "path": new_path,
                "mode": file_info["mode"],
                "type": file_info["type"],
            ...
            # 2. Eski dosya yolunu silinmek üzere işaretle
            new_tree.append({
                "path": path,
                "mode": file_info["mode"],
                "type": file_info["type"],
                "sha": None  # SHA'yı null yapmak dosyayı/yolu siler
            })

    if moved_files == 0:
        print("Uyarı: Taşınacak hiçbir dosya bulunamadı. Depo zaten düzenlenmiş olabilir.")
        return base_tree_sha # Değişiklik yoksa orijinal ağacı döndür

    url = f"https://api.github.com/repos/{REPO_OWNER}/{REPO_NAME}/git/trees"
    # Değişiklikleri temel ağaca göre gönderiyoruz
